...
@implementation CHSNavigationView
...
- (void)setModel:(CHSGradientNavigationModel *)model {
    ...
    [self.excuteButton setImage:[UIImage t_imageNamed:model.excuteImage] forState:UIControlStateNormal];
    ...
    [self.navigationView bringSubviewToFront:self.excuteButton];
}
...
- (void)setUI {
    ...
    [self.navigationView addSubview:self.excuteButton];
...
- (UIButton *)excuteButton {
    if(_excuteButton == nil) {
        _excuteButton = [[UIButton alloc] init];
        [_excuteButton addTarget:self action:@selector(toOperate: )forControlEvents:UIControlEventTouchUpInside];
        _excuteButton.tag = 2;
        _excuteButton.hidden = NO;
        [self setButton:_excuteButton fitSize:CGSizeMake(30,0)];
    }
